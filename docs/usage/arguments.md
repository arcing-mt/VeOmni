...
| moe_implementation | `str` | `"fused_triton"` | MoE experts forward implementation. `fused_triton` uses Triton group-gemm (GPU, SM70+); `fused_quack` uses Quack CUTLASS/CuTe (GPU, SM90+); `fused_musa` uses the MUSA group-gemm kernel; `fused_npu` uses the NPU group-gemm kernel; `eager` is the reference loop. A value still equal to the GPU default auto-resolves to `fused_npu` on NPU; explicit incompatible non-default overrides raise. |
| moe_dispatcher | `str` | `"alltoall"` | MoE token communication. `alltoall` is the default MCCL path; `deepep_ace` is the explicit MUSA DeepEP-ACE dispatch/combine path. It is independent of `moe_implementation`. |
| moe_deepep_num_sms | `int` | `20` | DeepEP communication resource setting, used only by `moe_dispatcher="deepep_ace"`; matches the reference `--moe-deepep-num-sms` default. |
| moe_deepep_token_capacity | `int` | `8192` | Per-rank ACE workspace token capacity. Set the same value on every EP rank and increase it explicitly for larger microbatches. |
| moe_shared_expert_overlap | `bool` | `false` | Qwen3.5-only: issue the independent shared expert inside the DeepEP-ACE dispatch window so its forward hides under the dispatch payload. Stays on the compute stream; requires `moe_dispatcher="deepep_ace"` and must be validated with a trace. |
| skip_empty_modality_dummy | `bool` | `true` | MUSA-only: skip a modality slot's `dummy_forward` when no rank holds that modality, instead of running a full extra vision-tower pass just to keep the FSDP collective counts level. Only bites on a single-modality dataset (e.g. image-only), where it halves the vision-tower passes per step; a batch holding both modalities is unaffected. Set to `false` to restore the generated forward's unconditional dummy. |
| vision_patch_embed_implementation | `str` | `"conv3d"` | MUSA-only, Qwen3.5-MoE only: ViT patch-embed implementation. `conv3d` (default) keeps the upstream `nn.Conv3d(3→1152, kernel=(2,16,16), stride=(2,16,16), padding=0)`. `linear` folds it into a single `F.linear` over the flattened `(temporal, patch, patch)` window — the same product, since `kernel == stride` and `padding == 0` make the convolution a plain matrix product. On MUSA the folded path fixes `implicit_gemm_conv3d_bwd_filter_ndhwc_128_128x128`, which caps occupancy at 13% and runs the weight gradient at ~0.7 TFLOP/s: the GEMM is ~216x faster per call (100.3 ms → 0.47 ms at 18k patches). The fold is **at least as accurate** — MUSA's conv3d kernel accumulates in reduced precision (7.3e-4 rms relative vs fp64, against 7.0e-7 for the GEMM) — but the two are not bit-identical, so an A/B loss curve differs in the last digits (≤1 bf16 ULP worst element). The choice is process-global and one-shot. Dense Qwen3.5 has the same patch embedder but is not wired yet, so this field is a no-op there. Set to `conv3d` to restore the generated forward; requesting `linear` off MUSA raises. |
...
| rms_norm_implementation | `str` | `"liger_kernel"` | RMSNorm. Known values: `liger_kernel` (default, GPU only), `musa` (MUSA-native fused), `npu`, `triton` (DeepSeek-V3 only; GPU only), `eager`. |
...
| rotary_pos_emb_implementation | `str` | `"liger_kernel"` | Rotary pos emb. Known values: `liger_kernel` (default, GPU only), `musa`, `npu`, `triton` (per-model: DeepSeek-V3, DeepSeek-V4, Wan; GPU only), `eager`. DeepSeek-V4 rejects `liger_kernel`, `npu`, and `musa`; Wan rejects `liger_kernel` and `musa` because their rotary layouts/signatures are model-specific. These incompatible choices raise at model registration, so their configs must pin a supported backend. |
| rotary_pos_emb_vision_implementation | `str` | `"eager"` | Vision rotary positional embedding. Known values: `eager`, `musa` , `npu`. |
...
| chunk_gated_delta_rule_implementation | `str` | `"fla"` | Chunk gated delta-rule kernel for Qwen3.5 linear attention. Known values: `eager`, `fla` (GPU/MLU/MUSA), `musa` (S5000-tuned FLA bridge, MUSA only), `flash_qla` (Hopper SM90), `npu` (requires `triton-ascend`). `eager` does not support varlen training. |
