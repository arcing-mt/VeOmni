...
| Rotary embedding | `rotary_pos_emb_implementation` | `eager`, `liger_kernel`, `musa`, `npu`, `triton` (per-model; DeepSeek-V3, DeepSeek-V4, Wan) | `"liger_kernel"` (GPU) | Model registration via ops config singleton |
...
| `musa` | torch-musa fused `torch.rms_norm` / `aten::_fused_rms_norm` | `torch_musa` |
...
| `musa` | torch-musa fused (`torch.rope`) | `torch_musa` |
...
| | VeOmni | Transformers v5 |
|---|--------|----------------|
| **Mechanism** | The per-model registry or variant-aware `OpSlot` selects `liger_kernel`, `musa`, `npu`, or a model-specific `triton` backend | `@use_kernel_forward_from_hub("rotary_pos_emb")` on the `apply_rotary_pos_emb` function (renamed from `use_kernel_func_from_hub` for standalone functions as of 5.16); `kernels` library downloads `apply_rotary_transformers` from `kernels-community/rotary`. The function is also attached to the Attention module via `@use_kernelized_func(apply_rotary_pos_emb)` so `kernelize()` can find it. |
