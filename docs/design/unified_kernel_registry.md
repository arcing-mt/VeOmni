...
| `skip_empty_modality_dummy` | boolean | MUSA-only and not a kernel selection: on by default, it skips a modality slot's `dummy_forward` when no rank holds that modality, halving the Qwen3.5-MoE vision-tower passes per step on a single-modality dataset. A no-op when a batch holds both modalities. See `veomni/models/transformers/qwen3_5_moe/qwen3_5_moe_musa_runtime_patch.py`. |
| `vision_patch_embed_implementation` | `conv3d`, `linear` | MUSA-only, Qwen3.5-MoE only, and not a registry kernel selection: `linear` (opt-in; default `conv3d`) rewrites the ViT patch embedder from `nn.Conv3d` to a single `F.linear` over the flattened `(temporal, patch, patch)` window. The same product, because `kernel == stride` and `padding == 0`; on MUSA the fold removes `implicit_gemm_conv3d_bwd_filter_ndhwc_128_128x128`, worth ~100 ms/step at 18k patches. The fold is at least as accurate as the conv kernel but not bit-identical, and the choice is process-global. Dense Qwen3.5 is not wired. Requesting `linear` off MUSA raises at config-parse time. See `veomni/models/transformers/qwen3_5_moe/qwen3_5_moe_musa_runtime_patch.py`. |
...
| `chunk_gated_delta_rule_implementation` | `eager`, `fla`, `musa`, `musa_tilelang`, `flash_qla`, `npu`, `npu_ascendc` | Qwen3.5 linear attention; `musa` applies the S5000-tuned FLA bridge; `musa_tilelang` uses the torch_kernels TileLang kernels and is packed-varlen training only; `flash_qla` is Hopper SM90-only. `npu` uses the vendored MindSpeed-MM Triton kernel; `npu_ascendc` is the AscendC fused `torch.ops.npu.*` path (requires a manual `fla_npu` install) |
