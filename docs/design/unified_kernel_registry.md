...
| `chunk_gated_delta_rule_implementation` | `eager`, `fla`, `musa`, `flash_qla`, `npu`, `npu_ascendc` | Qwen3.5 linear attention; `musa` applies the S5000-tuned FLA bridge; `flash_qla` is Hopper SM90-only. `npu` uses the vendored MindSpeed-MM Triton kernel; `npu_ascendc` is the AscendC fused `torch.ops.npu.*` path (requires a manual `fla_npu` install) |
