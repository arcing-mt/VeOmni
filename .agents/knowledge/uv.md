...
├── [project.optional-dependencies]     Hardware-shaped extras (four + legacy `dev`)
...
│   ├── conflicts            hardware extras are mutually exclusive
...
`gpu` / `musa` / `npu` / `npu_aarch64` are declared as conflicts. trl is not included
— VeOmni's DPO trainer is from-scratch.

```bash
uv sync --extra gpu --dev           # NVIDIA GPU
# MTT MUSA: use the base-image preservation workflow below
...
6. **`transformers==5.9.0` is the default supported version; MUSA uses
