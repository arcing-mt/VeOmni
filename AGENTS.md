...
group. `gpu` / `musa` / `npu` / `npu_aarch64` are mutually exclusive hardware
extras. The MUSA environment uses Python 3.10 and Transformers 5.17, so install
its extra with pip inside the base image as described in `.agents/knowledge/uv.md`.
